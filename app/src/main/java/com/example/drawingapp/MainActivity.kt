...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var ibBrush:ImageButton?=null
            ibBrush=findViewById(R.id.ib_brush)
        ibBrush?.setOnClickListener{
            showBrushSizeChoseDialog()
        }
        ...
        var ibRedo:ImageButton?=null
        ibRedo=findViewById(R.id.ib_redo)
        ibRedo?.setOnClickListener {
            drawingView?.onClickRedo()
        }
        var ibReset:ImageButton?=null
        ibReset=findViewById(R.id.ib_reset)
        ibReset.setOnClickListener {
            imageBackGround?.setImageResource(R.drawable.download1)
            drawingView?.clearScreen()
        }
    ...
    private fun showBrushSizeChoseDialog(){
        val brushDialog:Dialog=Dialog(this)
        brushDialog.setContentView(R.layout.dialog_brush_size)
        brushDialog.setTitle("BrushSize")
        var smallBtn:ImageButton? = null
        var mediumBtn:ImageButton?= null
        var largeBtn:ImageButton?=null
    ...
    private fun getBitmapFromView(view: View) : Bitmap {
        ...
        val bgDrawale = view.background

        bgDrawale.draw(canvas)
        view.draw(canvas)
        ...
        withContext(Dispatchers.IO){
            if (mBitmap != null) {
                try {
                    ...
                    val f =
                        File(externalCacheDir?.absoluteFile.toString() + File.separator + "KidDrawingApp" + System.currentTimeMillis() / 1000 + ".jpeg")
                    ...
                    runOnUiThread {
                        ...
                        if (result.isNotEmpty()) {
                            Toast.makeText(
                                this@MainActivity,
                                "File is saved successfuly : $result",
                                Toast.LENGTH_SHORT
                            ).show()
                        shareImage(result)
                        }
